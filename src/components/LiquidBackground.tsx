...
class GradientBackground {
    ...
    constructor(sceneManager: any) {
        ...
        // Converted tbuzz Midnight Glass neon colors to normalize RGB (0-1) range 
        // hsl(263, 70%, 50%) -> roughly rgb(168, 85, 247) -> (0.66, 0.33, 0.97)
        // hsl(330, 81%, 60%) -> roughly rgb(236, 72, 153) -> (0.92, 0.28, 0.6)
        // Violet/White highlights
        this.uniforms = {
            uTime: { value: 0 },
            uResolution: { value: new THREE.Vector2(window.innerWidth, window.innerHeight) },
            uColor1: { value: new THREE.Vector3(0.66, 0.33, 0.97) }, // Purple
            uColor2: { value: new THREE.Vector3(0.92, 0.28, 0.60) }, // Pink
            uColor3: { value: new THREE.Vector3(0.66, 0.33, 0.97) }, // Purple
            uColor4: { value: new THREE.Vector3(0.92, 0.28, 0.60) }, // Pink
            uColor5: { value: new THREE.Vector3(0.50, 0.10, 0.80) }, // Deep violet
            uColor6: { value: new THREE.Vector3(0.98, 0.70, 0.90) }, // Light pink/white
            uSpeed: { value: 1.2 }, uIntensity: { value: 1.8 },
            uTouchTexture: { value: null }, uGrainIntensity: { value: 0.08 },
            uDarkNavy: { value: new THREE.Vector3(0.0, 0.0, 0.0) }, // Pure Black background
            uGradientSize: { value: 0.45 }, uGradientCount: { value: 12.0 },
            uColor1Weight: { value: 0.8 }, uColor2Weight: { value: 1.4 } // Rebalanced for black bg
        };
    }
    init() {
        ...
        vec3 getGradientColor(vec2 uv, float time) {
          ...
          // Darker background merge for pure black aesthetic
          color = mix(uDarkNavy, color, max(brightness * 1.0, 0.05));
          return color;
        }
...
export function LiquidBackground({ isPlaying = true }: LiquidGradientProps) {

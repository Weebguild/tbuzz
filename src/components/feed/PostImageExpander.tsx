...
import { NeonSparkOverlay } from "./NeonSparkOverlay";
    ...
    >
      ...
      <div
        className={`relative flex items-center justify-center w-full overflow-hidden ${
          isSplitScreen ? "h-[45vh] border-b border-white/10" : "h-screen"
        }`}
      >
        <button
        ...
        >
          <img src={imageUrl} alt="Expanded" className="w-full h-full object-contain pointer-events-none" />
        </NeonSparkOverlay>
        ...
        >
          <button onClick={handleLikeClick} className="flex items-center gap-2 text-white group">
            <Heart
              className={`h-6 w-6 transition-all duration-300 ${localLiked ? "fill-accent text-accent drop-shadow-[0_0_15px_hsl(var(--accent)/0.8)] scale-110" : "group-hover:text-accent"}`}
            />
          ...
          <button
            ...
            className={`flex items-center gap-2 transition-colors ${isSplitScreen ? "text-primary drop-shadow-[0_0_10px_hsl(var(--primary)/0.8)]" : "text-white hover:text-primary"}`}
          >
      ...
      <AnimatePresence>
        {isSplitScreen && (
          <motion.div
            ...
            className="flex-1 bg-gradient-to-b from-[hsl(var(--card))] to-black flex flex-col overflow-hidden relative"
          >
            ...
            <div className="flex-1 overflow-y-auto px-4 py-6 space-y-5 no-scrollbar pb-24">
              {loadingComments ? (
                ...
                <div className="text-center py-10">
                  <p className="text-muted-foreground/60 text-sm font-medium">No comments yet. Start the conversation.</p>
                </div>
              ) : (
                comments.map((c) => (
